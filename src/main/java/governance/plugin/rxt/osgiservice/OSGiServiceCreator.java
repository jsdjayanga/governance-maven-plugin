...
public class OSGiServiceCreator extends AbstractAssetCreator {
    ...
    public boolean create(Map<String, Object> parameters) throws MojoExecutionException {
        ...
        String OSGiServicePath = getResourcePath(new String[]{className, namespace});

        String createServiceRequst =
                GovernanceSOAPMessageCreator.createAddOSGiServiceRequest(className
                        , namespace
                        , version
                        , description
                        ,references);
        ...
        if (logger.isInfoEnabled()){
            if (isOSGiServiceCreated){
                logger.info("Request sent to create Service: "+ className);
            }else{
                logger.info("Service already available: " + className);
            }
        }
